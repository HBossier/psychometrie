...
\begin{itemize}
	...
	\item De zware delinquenten in kwestie  zitten verspreid over 50 instellingen waarbij er per instelling slechts twee psychologen beschikbaar zijn. 
	Per instelling behandelen de twee psychologen dezelfde kaarten $1-3$. door $p_1$ gescoord en kaarten $4-6$ door $p_2$; in de andere instelling ($p_3$ en $p_4$)werden kaarten $4-6$ gescoord door $p_4$ en de kaarten $1-3$ door $p_3$. 
\end{itemize}
\item Bereken voor het eerste scenario uit 4 de $G$-co\"{e}ffici\"{e}nt en voor het tweede scenario de index of dependability $\phi$. Waar ligt het verschil tussen beide maten?
\end{enumerate}
}
...
\input{fig_gen2}
\item JUIST. In een volledig gekruist opzet kunnen steeds meer variantiecomponenten onderscheiden worden dan in om het even welk genest opzet dat gebruik dezelfde meetfacetten. 
\item
\begin{itemize}
	...
	$\sigma$  			& $ 167.64 $ 			& $ 3.211 $ 			& $615.8+1.3  $ 			& $ .$				 & $. $	& $ 84.7 $	 & $ 1.2 + 1.3  $ \\
	$n_.$				& .						& 6						& 3*6				 		& $ .$		  		 & $. $	& $ 3 $		 &  3*6 \\ \hline
	$\sigma / n$ 		& .						& 0.5352				& 34.2833				 	& $ .$			  	 & $. $	& $ 28.2333 $& 0.1389 \\ \hline
	\end{tabular} \\
	De grafische weergave van dit opzet is het volgende:\\
	\input{fig_gen2A}
	
	\item Het opzet wordt hier: $p\left(k\times d\right)$ bijgevolg kunnen de variantiecomponenten $\hat{\sigma}^2_p$ en $\hat{\sigma}^2_{kp}$, $\hat{\sigma}^2_{pd}$ niet onderscheiden worden van  van de residuele variantie-term worden onderscheiden (wordt $\hat{\sigma}^2_{p, kp, pd, dpk, e}$). Houd rekening met het verminderde aantal kaarten ($K$).\\Dit zijn de te onderscheiden componenten:\\ Merk hierbij opnieuw op dat het object van meting $d$ is. \\

	\begin{tabular}{|c|c|c|c|c|c|c|c|} \hline
	 & $ \hat{\sigma}^2_{d} $ & $ \hat{\sigma}^2_{k} $& $ \hat{\sigma}^2_{p} $ & $ \hat{\sigma}^2_{dk} $ & $ \hat{\sigma}^2_{dp} $ & $ \hat{\sigma}^2_{kp} $ &
	 $\sigma^2_{p, kp, dp, dpk, e}$ \\ \hline
	$\sigma$  			& $167.64$ 			& $ 3.211 $  	& . 	  & $1.3$				 & $ . $	& $ . $& $ 1.2 + 1.3 +  615.8 +84.7$ \\
	$n_.$ 				& . 					& $3$			& .	  & $ 3$		  		 & $ . $ 	& $ . $&  $2 *3$ \\ \hline
	$\sigma / n_.$      & .						& $1.0703 $		& .   & $0.4333$			  	 & $.$	& $ . $&  103.2667 \\ \hline
	\end{tabular} \\
	De grafische weergave van dit opzet is het volgende:\\
...
\begin{itemize}
	...
	$\bm{\hat{\sigma}^2_p},\bm{\hat{\sigma}^2_{dp}} , \bm{\hat{\sigma}^2_{kp}} en \bm{\hat{\sigma}^2_{dkp, e}}$.\\
	...
	\end{align*}
	Bijgevolg kan $\phi$ opgesteld worden door $\sigma^2_{abs}$ en $\hat{\sigma}^2_{d}$ in te vullen in vergelijking \ref{eq.phi}:
	\begin{align*}
